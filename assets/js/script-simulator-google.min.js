...
$(this).not('a').data("clicks",!clicks);header.toggleClass('header--active');if(header.hasClass('header--active')){$('body').css('overflow','hidden');$('.menu li a[href]').on('click',function(){$('.header').removeClass('header--active');$('body').css('overflow','auto')})}})});$(function(){$(window).on("scroll",function(){if($(window).scrollTop()){$('.header').addClass('header__pages--sticky')}else{$('.header').removeClass('header__pages--sticky')}})});$(function(){const sliderCredit=$('#credit');const outputCredit=$('#credit-value');const sliderSales=$('#sales');const outputSales=$('#sales-value');const data=$('.data-description');outputCredit.html(sliderCredit.val());sliderCredit.on('input',function(){outputCredit.html($(this).val())});outputSales.html(sliderSales.val());sliderSales.on('input',function(){outputSales.html($(this).val())});sliderSales.on('input',function(){if($(this).val()>1){data.html('vendas')}else{data.html('venda')}})});$(document).ready(function(){const path=$(location).attr("href").split("simulador-google/",1).reverse().join('');const file='wp-content/themes/goognet/template-parts/simulator-google/simulator-result.php';$('#simulator').submit(function(){$(".data__status").removeClass("d-none");$(".result").addClass("d-none");$('#button-simulator').attr('disabled',!0);$.ajax({url:path.concat(file),type:'POST',cache:!1,data:$('#simulator').serialize(),success:function(data){$('.result__container').html(data);setTimeout(function(){$(".data__status").addClass("d-none");$(".result").removeClass("d-none");$('#button-simulator').attr('disabled',!1);$('html,body').animate({scrollTop:$('.result').offset().top-$('.header').innerHeight()},500)},3000);if($('.form').hasClass('d-none')){$('.result__content__row').css('filter','none')}},error:function(){$('.result').html('Erro')}});return!1})});$(document).ready(function(){const pathSender=$(location).attr("href").split("simulador-google/",1).reverse().join('');const fileSender='wp-content/themes/goognet/source/Support/Sender-SimulatorGoogle.php';$('#form-simulator').submit(function(){$('.form__content__group__status').removeClass('d-none');$.ajax({url:pathSender.concat(fileSender),type:'POST',cache:!1,data:$('#form-simulator').serialize(),success:function(data){$('.form').html(data);setTimeout(function(){if(!$('.form').hasClass('error')){$('.form__content__group__status').addClass('d-none');$('.form').addClass('d-none');$('.result__content__row').css('filter','none')}},3000)},error:function(){$('.form').html('Erro ao enviar')}});return!1})});/*!
 * sweetalert2 v7.0.8
 * Released under the MIT License.
 */
