...
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(document).ready(function(){const path=$(location).attr("href");const file='wp-content/themes/goognet/source/Support/Sender.php';$('#form').submit(function(){$(".contact__row__content__form__status").removeClass("d-none");$.ajax({url:path.concat(file),type:'POST',cache:!1,data:$('#form').serialize(),success:function(data){$('.contact__row__content__form__status').append(data);setTimeout(function(){$(".contact__row__content__form__status").addClass("d-none")},3000)},error:function(){$('.contact__row__content__form__status').append('Erro')}});return!1})});const screenPosition=window.innerHeight/1.25;function animationAbout(){const about=document.querySelector('.about__content');const aboutPosition=about.getBoundingClientRect().top;if(aboutPosition<screenPosition){about.classList.add('about__content--animation')}}
window.addEventListener('scroll',animationAbout);function animationSimulador(){const simulador=document.querySelector('.simulator__row__content__body');const simuladorPosition=simulador.getBoundingClientRect().top;if(simuladorPosition<screenPosition){simulador.classList.add('simulator__row__content__body--animation')}}
window.addEventListener('scroll',animationSimulador);function animationBlog(){const blog=document.querySelector('.blog__content__container__body');const blogPosition=blog.getBoundingClientRect().top;if(blogPosition<screenPosition){blog.classList.add('blog__content__container__body--animation')}}
