...
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(document).ready(function(){const path=$(location).attr("href");const file='wp-content/themes/tropicalpet/source/Support/Sender.php';$('#form').submit(function(){$.ajax({url:path.concat(file),type:'POST',data:$('#form').serialize(),success:function(data){$('.contact__form__row__form__status').html(data)}});return!1})});$(function(){$("#button").click(function(){$(".contact__form__row__form__status--loading").removeClass("d-none")})})