...
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(document).ready(function(){const path=$(location).attr("href");const file='wp-content/themes/goognet/source/Support/Sender.php';$('#form').submit(function(){$(".contact__row__content__form__status--loading").removeClass("d-none");$.ajax({url:path.concat(file),type:'POST',data:$('#form').serialize(),success:function(data){$('.contact__row__content__form__status').append(data);$('#button-contact').on('click',function(){setTimeout(function(){$(".contact__row__content__form__status--loading").addClass("d-none");$(".result").removeClass("d-none")},3000)})},error:function(){$('.contact__row__content__form__status').append('Erro')}});return!1})});const screenPosition=window.innerHeight/1.25;function animationAbout(){const about=document.querySelector('.about__content');const aboutPosition=about.getBoundingClientRect().top;if(aboutPosition<screenPosition){about.classList.add('about__content--animation')}}
