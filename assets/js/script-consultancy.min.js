...
$(this).not('a').data("clicks",!clicks);header.toggleClass('header--active');if(header.hasClass('header--active')){$('body').css('overflow','hidden');$('.menu li a[href]').on('click',function(){$('.header').removeClass('header--active');$('body').css('overflow','auto')})}})});$(function(){$(window).on("scroll",function(){if($(window).scrollTop()){$('.header').addClass('header--sticky')}else{$('.header').removeClass('header--sticky')}})});$(document).ready(function(){const pathSender=$(location).attr("href").split("consultoria/",1).reverse().join('');const fileSender='wp-content/themes/goognet/source/Support/Sender-Consultoria.php';$('#form').submit(function(){$(".hero__row__form__container__content__group__status").removeClass("d-none");$.ajax({url:pathSender.concat(fileSender),type:'POST',cache:!1,data:$('#form').serialize(),success:function(data){$('.hero__row__form__container__content__group__status').append(data);setTimeout(function(){$(".hero__row__form__container__content__group__status").addClass("d-none");$('#button-sender').attr('disabled',!1)},3000)},error:function(){$('.hero__row__form__container__content__group__status').append('Erro')}});return!1})});/*!
 * sweetalert2 v7.0.8
 * Released under the MIT License.
 */
