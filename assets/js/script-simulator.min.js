...
window.addEventListener('scroll',animationBlog);$(function(){const sliderCredit=$('#credit');const outputCredit=$('#credit-value');const sliderSales=$('#sales');const outputSales=$('#sales-value');const data=$('.data-description');outputCredit.html(sliderCredit.val());sliderCredit.on('input',function(){outputCredit.html($(this).val())});outputSales.html(sliderSales.val());sliderSales.on('input',function(){outputSales.html($(this).val())});sliderSales.on('input',function(){if($(this).val()>1){data.html('vendas')}else{data.html('venda')}})});$(document).ready(function(){const path=$(location).attr("href").split("simulador/",1).reverse().join('');const file='wp-content/themes/goognet/template-parts/simulator/simulator-result.php';$('#simulator').submit(function(){$.ajax({url:path.concat(file),type:'POST',data:$('#simulator').serialize(),success:function(data){$('.result').html(data)},error:function(){$('.result').html('Erro')}});return!1})});$(function(){$("#button-simulator").click(function(){$(".data__status").removeClass("d-none")})})