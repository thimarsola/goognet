...
$(this).not('a').data("clicks",!clicks);header.toggleClass('header--active');if(header.hasClass('header--active')){$('body').css('overflow','hidden');$('.menu li a[href]').on('click',function(){$('.header').removeClass('header--active');$('body').css('overflow','auto')})}})});$(function(){$(window).on("scroll",function(){if($(window).scrollTop()){$('.header').addClass('header__pages--sticky')}else{$('.header').removeClass('header__pages--sticky')}})});const screenPosition=window.innerHeight/1.25;function animationBlog(){const blog=document.querySelector('.hero__content__container__body');const blogPosition=blog.getBoundingClientRect().top;if(blogPosition<screenPosition){blog.classList.add('hero__content__container__body--animation')}}
window.addEventListener('scroll',animationBlog);$(function(){const sliderCredit=$('#credit');const outputCredit=$('#credit-value');const sliderSales=$('#sales');const outputSales=$('#sales-value');const data=$('.data-description');outputCredit.html(sliderCredit.val());sliderCredit.on('input',function(){outputCredit.html($(this).val())});outputSales.html(sliderSales.val());sliderSales.on('input',function(){outputSales.html($(this).val())});sliderSales.on('input',function(){if($(this).val()>1){data.html('vendas')}else{data.html('venda')}})});$(document).ready(function(){const path=$(location).attr("href").split("simulador/",1).reverse().join('');const file='wp-content/themes/goognet/template-parts/simulator/simulator-result.php';$('#simulator').submit(function(){$(".data__status").removeClass("d-none");$(".result").addClass("d-none");$('.modal').addClass('d-none');$('#button-simulator').attr('disabled',!0);$.ajax({url:path.concat(file),type:'POST',cache:!1,data:$('#simulator').serialize(),success:function(data){$('.result').html(data);setTimeout(function(){$(".data__status").addClass("d-none");$(".result").removeClass("d-none");$('#button-simulator').attr('disabled',!1);$('html,body').animate({scrollTop:$('.result').offset().top},500)},3000);$('#modal').on('click',function(){const segment=$('.segment').html();const credit=$('.credit').html();const sales=$('.sales').html();const ticket=$('.ticket').html();const clicks=$('.result__content__row__information__clicks p').html();const budget=$('.result__content__row__information__budget p').html();const totalSales=$('.result__content__row__information__sales p').html();const roi=$('.result__content__row__roi__container p').html();$('.modal').removeClass('d-none');$('body').css('overflow','hidden');$('#contact-segment').val(segment);$('#contact-credit').val(credit);$('#contact-sales').val(sales);$('#contact-ticket').val(ticket);$('#contact-clicks').val(clicks);$('#contact-budget').val(budget);$('#contact-total-sales').val(totalSales);$('#contact-roi').val(roi)})},error:function(){$('.result').html('Erro')}});return!1})});$(document).ready(function(){const pathSender=$(location).attr("href").split("simulador/",1).reverse().join('');const fileSender='wp-content/themes/goognet/source/Support/Sender-Simulator.php';$('.close').on('click',function(){$('.modal').addClass('d-none');$('body').css('overflow','auto')});$('#form-simulator').submit(function(){$('.modal').removeClass('d-none');$(".modal__content__group__status").removeClass("d-none");$.ajax({url:pathSender.concat(fileSender),type:'POST',cache:!1,data:$('#form-simulator').serialize(),success:function(data){$('.modal').append(data);setTimeout(function(){$(".modal__content__group__status").addClass("d-none");$('#button-simulator').attr('disabled',!1);$('.modal').addClass('d-none');$('body').css('overflow','auto')},3000)},error:function(){$('.modal').append('Erro')}});return!1})});/*!
 * sweetalert2 v7.0.8
 * Released under the MIT License.
 */
